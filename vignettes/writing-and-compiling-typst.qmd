...
`{rtyp}` provides functions to help you write and compile your Typst markup. 
...
`{rtyp}` also offers a `is_valid_typst()` that returns `TRUE`/`FALSE` depending on if the code can be compiled or not. Check out that the [dedicated vignette](../typst-validation.hml).
