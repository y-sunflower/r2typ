...
```{r}
heading(level = 2, "A title") |> print()
text(weight = 500, "Some text") |> print()
```
...
```{r}
list_(tight = FALSE, "hey", "you", "!") |> print()
```
...
```{r}
text(size = pt(20), "Ice cream") |> print()
```

This works with all Typst units:

```{r}
image("image.png", width = percent(80)) |> print()
```
...
```{r}
text(fill = green, "Green text") |> print()
```
...
```{r}
text(fill = rgb("#ffc300"), "Yellow-ish text") |> print()
```
...
```{r}
image("image.png", width = percent(80), alt = NULL) |> print()
```
...
```{r}
text(`stylistic-set` = c(1, 2, 3), "10 years ago") |> print()

text(`stylistic-set` = list(1, 2, 3), "10 years ago") |> print() # equivalent
```

- Named `list()` (such as `list(a = "hello", b = "world")`) become dictionnaries:

```{r}
text(costs = list(hyphenation = percent(100), runt = percent(100))) |> print()
```
...
```{r}
show_heading(set_text(navy)) |> print()
```
