...
`{rtyp}` is an R package designed to generate **Typst markup**. You can use it as is, but it also works well with [Quarto](https://quarto.org/).
...
However, in Typst, not all functions behave the same way regarding positional and named arguments. For this reason, `{rtyp}` automatically adjusts the behavior when necessary.
